...
export default makeStyles(
  ({ palette, boxShadows, functions, transitions, breakpoints, borders, typography }: any) => {
    ...
    return {
      navbar: {
        ...
        color: ({ transparentNavbar, light }: any) => {
          let color;
        ...
        top: ({ absolute }: any) => (absolute ? 0 : pxToRem(12)),
        ...
        paddingRight: ({ absolute }: any) => (absolute ? pxToRem(8) : 0),
        paddingLeft: ({ absolute }: any) => (absolute ? pxToRem(16) : 0),
      ...
      navbar_row: {
        ...
        [breakpoints.up("md")]: {
          justifyContent: ({ isMini }: any) => (isMini ? "space-between" : "stretch"),
          width: ({ isMini }: any) => (isMini ? "100%" : "max-content"),
        },
