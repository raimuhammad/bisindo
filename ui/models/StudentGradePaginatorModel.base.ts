 ...
 */
export const StudentGradePaginatorModelBase = withTypedRefs<Refs>()(
  ModelBase.named("StudentGradePaginator")
    .props({
      __typename: types.optional(
        types.literal("StudentGradePaginator"),
        "StudentGradePaginator"
      ),
      /** Pagination information about the list of items. */
      paginatorInfo: types.union(
        types.undefined,
        types.late((): any => PaginatorInfoModel)
      ),
      /** A list of StudentGrade items. */
      data: types.union(
        types.undefined,
        types.array(MSTGQLRef(types.late((): any => StudentGradeModel)))
      ),
    })
    .views((self) => ({
      get store() {
...
export class StudentGradePaginatorModelSelector extends QueryBuilder {
  paginatorInfo(
    builder?:
      ...
      | ((selector: PaginatorInfoModelSelector) => PaginatorInfoModelSelector)
  ) {
    return this.__child(`paginatorInfo`, PaginatorInfoModelSelector, builder);
  }
  data(
    builder?:
      | string
      | StudentGradeModelSelector
      | ((selector: StudentGradeModelSelector) => StudentGradeModelSelector)
  ) {
    return this.__child(`data`, StudentGradeModelSelector, builder);
  }
}
...
export const studentGradePaginatorModelPrimitives =
  selectFromStudentGradePaginator();
