 ...
 */
export const StudentGradeModelBase = withTypedRefs<Refs>()(
  ModelBase.named("StudentGrade")
    .props({
      ...
      grade: types.union(
        types.undefined,
        MSTGQLRef(types.late((): any => GradeModel))
      ),
      progress: types.union(
        types.undefined,
        MSTGQLRef(types.late((): any => ProgressModel))
      ),
    })
    .views((self) => ({
      get store() {
...
export class StudentGradeModelSelector extends QueryBuilder {
  ...
  student(
    ...
    return this.__child(`student`, UserModelSelector, builder);
  }
  grade(
    builder?:
      | string
      | GradeModelSelector
      | ((selector: GradeModelSelector) => GradeModelSelector)
  ) {
    return this.__child(`grade`, GradeModelSelector, builder);
  }
  progress(
    builder?:
      | string
      | ProgressModelSelector
      | ((selector: ProgressModelSelector) => ProgressModelSelector)
  ) {
    return this.__child(`progress`, ProgressModelSelector, builder);
  }
}
export function selectFromStudentGrade() {
  return new StudentGradeModelSelector().student(
    (b) => b.id.name.active.email.need_change_password.created_at
  );
}

export const studentGradeModelPrimitives =
  selectFromStudentGrade().created_at.updated_at;
